...
<template>
  ...
  >
    <div class="flex-cc !hidden max-[1180px]:!flex ml-2 max-sm:ml-6">
      <img v-if="siteConfig.logo" :src="siteConfig.logo" class="w-10 h-10 rounded-lg object-contain" />
      <h1 class="text-xl ont-mediumf ml-2">{{ siteConfig.sitename || AppConfig.systemInfo.name }}</h1>
    </div>

    <div class="flex-cc gap-1.5 mr-2 max-sm:mr-5 max-sm:!hidden">
...
<script setup lang="ts">
  ...
  import { siteConfig } from '@/store/modules/site'
